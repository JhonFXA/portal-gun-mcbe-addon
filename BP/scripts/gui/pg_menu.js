...
function stopPlayerAnimation(player) {
  player.playAnimation("animation.ram_pg.player.reset");
}
...
export function openPortalGunMenu(player, portalGunItem, gunInstance = null) {
  ...
  const customUi = new ActionFormData()
    ...
    .button("", "textures/ram_pg/ui/pg_ui/menu/unplug_tube");

  playPlayerAnimation(player);
  player.dimension.playSound("ram_pg:open_menu", player.location);
...
function openSavedLocationsForm(player, inventory, portalGunItem) {
  ...
  const form = new ActionFormData()
    ...
    .button(
      ...
      "textures/ram_pg/ui/pg_ui/saved_locations/save_ui",
    )
    .button(
      "Delete Location",
  ...
  const locationsJson = portalGunItem.getDynamicProperty(
  ...
  if (savedLocations.length > 0) {
    ...
    savedLocations.forEach((location) => {
      ...
      form.button(
        `§0${name}§r\nX: ${x}, Y: ${y}, Z: ${z}`,
        `textures/ram_pg/ui/pg_ui/saved_locations/${dimName}`,
      );
    });
  } else {
 ...
 */
function openFilteredLocationsForm(
  player,
  inventory,
  portalGunItem,
  filteredLocations,
  ...
  resultsForm
    .divider()
    .button("Back to Saved Locations", "textures/ram_pg/ui/pg_ui/back_button");
 ...
 * @param {Array} savedLocations
 */
function openSaveCurrentLocationForm(
  player,
  inventory,
  portalGunItem,
  savedLocations,
 ...
 * @param {Array} savedLocations
 */
function openDeleteLocationForm(
  player,
  inventory,
  portalGunItem,
  savedLocations,
  ...
  if (savedLocations.length === 0) {
    player.onScreenDisplay.setActionBar(`§cNo saved locations to delete.§r`);
    player.dimension.playSound("ram_pg:error_sound", player.location);
    stopPlayerAnimation(player);
    return;
  }
  ...
  form.divider().button("Cancel", "textures/ram_pg/ui/pg_ui/back_button");
...
function openSetCoordinatesForm(player, inventory, portalGunItem) {
  ...
  form.show(player).then((response) => {
    ...
    if (Number.isNaN(x) || Number.isNaN(y) || Number.isNaN(z) || !dimensionId) {
      ...
      player.onScreenDisplay.setActionBar(`§cCoordinates out of bounds.§r`);
      player.dimension.playSound("ram_pg:error_sound", player.location);
    } else {
      const newLocationData = {
...
function openSettingsForm(player, inventory, portalGunItem) {
  ...
  let form = new ActionFormData()
    ...
    .button("Behavior Settings", "textures/ram_pg/ui/pg_ui/settings/toggle")
    .button("Back to Menu", "textures/ram_pg/ui/pg_ui/back_button")
    .divider()
...
function openBehaviorSettingsForm(player, portalGunItem, inventory) {
  ...
  form.show(player).then((response) => {
    if (response.formValues == undefined) {
      ...
      player.onScreenDisplay.setActionBar(`§aSettings updated.§r`);
      player.dimension.playSound("ram_pg:selection", player.location);
      stopPlayerAnimation(player);
    }
  });
...
function openHistoryForm(player, inventory, portalGunItem) {
  ...
  let form = new ActionFormData()
    .title("History")
    .body(
      "Here you can view your §eportal gun's teleportation history§r.\nSelect a location below to use it.\n\n§eLimit: 30 locations§r",
    )
    .divider()
    .button("Back to Settings", "textures/ram_pg/ui/pg_ui/back_button")
    .divider();
...
function openResetForm(player, portalGunItem, inventory) {
  ...
  form.show(player).then((response) => {
    ...
    if (response.selection == 0) {
      ...
      const bootleg = portalGunItem.getDynamicProperty(
      ...
      player.dimension.playSound("ram_pg:selection", player.location);
      player.onScreenDisplay.setActionBar(`§aPortal gun has been reset.§r`);
    } else if (response.selection == 1) {
      openSettingsForm(player, inventory, portalGunItem);
    }
  });
...
const terminalCommands = {
  ...
  gc: (player, inventory, portalGunItem) => {
    const debug = getGunConfig(player, inventory, portalGunItem);
    openTerminalForm(player, inventory, portalGunItem, debug);
  },
  help: (player, inventory, portalGunItem) => {
    const helpText = `Available commands:
    - gunconfig | gc : Show Portal Gun info
...
Dimension: ${color}${dimName}§r`;
  ...
  save: (player, inventory, portalGunItem, locationName) => {
    ...
    const savedLocationsJson = portalGunItem.getDynamicProperty(
      portalGunDP.savedLocations,
    );
    const savedLocations = savedLocationsJson
  ...
  delloc: (player, inventory, portalGunItem, locationName) => {
    if (!locationName) {
      openTerminalForm(
        ...
        inventory,
        portalGunItem,
        "§cUsage: delloc <name> or delloc -all",
      );
      return;
    }
    const savedLocationsJson = portalGunItem.getDynamicProperty(
    ...
    let savedLocations = savedLocationsJson
      ? JSON.parse(savedLocationsJson)
      : [];
    if (locationName === "-all") {
      if (savedLocations.length === 0) {
      ...
      openTerminalForm(
        ...
        inventory,
        portalGunItem,
        `§aDeleted all saved locations.§r`,
      );
      return;
    }
    ...
    if (savedLocations.length === initialLength) {
      openTerminalForm(
        ...
        inventory,
        portalGunItem,
        `§cNo location found with name '${locationName}'.§r`,
      );
      return;
    }
    portalGunItem.setDynamicProperty(
    ...
    openTerminalForm(
      ...
      inventory,
      portalGunItem,
      `§aDeleted location '${locationName}'.§r`,
    );
  },
};
...
function getGunConfig(player, inventory, portalGunItem) {
  ...
  const safePlacement = portalGunItem.getDynamicProperty(
    ...
    ? true
    : false;
  const bootlegFluid = portalGunItem.getDynamicProperty(
    portalGunDP.bootleggedFluid,
  )
    ? true
    : false;
...
function getDimensionLabel(dimensionId) {
  switch (dimensionId) {
    case "minecraft:overworld":
      return { dimName: "Overworld", color: "§q" };
    case "minecraft:nether":
      return { dimName: "Nether", color: "§4" };
    case "minecraft:the_end":
      return { dimName: "The End", color: "§u" };
    default:
      return { dimName: "Unknown", color: "§f" };
  }
}
...
function dismountPortalGun(player, portalGunItem, inventory) {
  ...
  const charge = portalGunItem.getDynamicProperty(portalGunDP.charge);
  const isBootlegged = portalGunItem.getDynamicProperty(
    portalGunDP.bootleggedFluid,
  );
  ...
  if (charge > 0) {
    const chargedTube = new ItemStack(
      isBootlegged ? bootlegTubeId : chargedTubeId,
      1,
    );
